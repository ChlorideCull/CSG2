...
# -*- coding: utf-8 -*-
"""
Stripped down bottle.py, with only Templates and Routes left.

Homepage and documentation for the real deal: http://bottlepy.org/

Copyright (c) 2015, Marcel Hellkamp.
Butchered in 2016 by Dusty.
...
def abort(code=500, text='Unknown Error.'):
    ...
    raise Exception("{} {}".format(code, text))
...
class Router(object):
    ...
    def _itertokens(self, rule):
        ...
        for match in self.rule_syntax.finditer(rule):
            ...
            g = match.groups()
            if g[2] is not None:
                raise Exception("Invalid route syntax")
...
class BaseTemplate(object):
    ...
    def __init__(self,
        ...
        if not self.source and self.name:
            ...
            if not self.filename:
                raise Exception('Template %s not found.' % repr(name))
    ...
    def search(cls, name, lookup=None):
        ...
        if not lookup:
            raise Exception("Empty template lookup path.")
...
class SimpleTemplate(BaseTemplate):
    ...
    def code(self):
        ...
        try:
            ...
            raise Exception("Template isn't UTF-8.")
...
class StplSyntaxError(Exception):
    pass
...
class StplParser(object):
    ...
    _re_tok = r'''((?mx)         # verbose and dot-matches-newline mode
        [urbURB]*
        (?:  ''(?!')
            |""(?!")
            |'{6}
